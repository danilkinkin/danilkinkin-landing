...
export function AnimatedBlock({
  ...
  return (
    <span
      ...
      className={clsx(
        styles.textLine,
        !noSpaceAfter && styles.spaceAfter,
        className,
      )}
    >
...
export function AnimateOrder({ delay = 0, children }) {
  ...
  return React.Children.toArray(children).map((children, index) => {
    if (children.type !== AnimatedBlock && !("animatedBlock" in children.props))
      return children;
    ...
    return React.createElement(children.type, {
      ...
      delay:
        (comulutiveDelay - children.props.value.length) * delayBeetwenChars +
        delay,
    });
  });
}
